...
<body>
    ...
    form.addEventListener('submit', function (event) {
        ...
        stripe.createToken(card).then(function (result) {
            if (result.error) {
                ...
                fetch('/expresso-cafe/api/process_payment_order', { // Updated endpoint for order payments
                    method: 'POST',
                    headers: {
                ...
                .then(function (data) {
                    if (data && data.success) {
                        ...
                        if (data.message.includes("failed to create receipt")) {
                            showToast("Payment successful, but receipt creation failed. Please contact support.", 'warning');
                        } else {
                ...
                .catch(error => {
                    ...
                    if (error.message === "Order is already marked as paid.") {
                        showToast("This order is already paid. No further payment is needed.", 'info');
                    } else if (error.message.includes("receipt text")) {
                        showToast("Payment was successful, but the receipt text could not be generated. Contact support for assistance.", 'warning');
                    } else {
    ...
    function showToast(message, type) {
        ...
        toast.style.backgroundColor = type === 'success' ? '#28a745' : type === 'info' ? '#17a2b8' : type === 'warning' ? '#ffc107' : '#dc3545';
